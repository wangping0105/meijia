module Api::IkParamsProcessable
  ...
  # 只要flag 设为true，每次都需要执行中间的代码，这是为了防止在同一个action调用多个不同的 params
  def params_encoded(params, flag = false)
    if (@params_encoded.blank? || flag)
      params_encoded = Utility.deep_transform_values(params) {|v|
      # params_encoded = params.each{|k,v| 这是个坑，只遍历最外层
        if v.is_a?(String)
          ...
          v = Emojimmy.emoji_to_token(v)
        end
        v
      }
      @params_encoded = HashWithIndifferentAccess.new(params_encoded)
    end
    @params_encoded
  end
end
